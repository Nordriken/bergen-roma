...
const RUTE = [
  ...
  { navn:'Borgund',     lat:61.0500, lng:7.8100 },
  { navn:'Fagernes',    lat:60.9877, lng:9.2333 },
  {                     lat:60.1683, lng:10.2586 },
  { navn:'Oslo',        lat:59.9139, lng:10.7522 },
  { navn:'Halden',      lat:59.1256, lng:11.3878 },
  {                     lat:58.3479, lng:11.9424 },
  { navn:'Göteborg',    lat:57.7089, lng:11.9746 },
  { navn:'Halmstad',    lat:56.6739, lng:12.8578 },
  { navn:'Malmö',       lat:55.6050, lng:13.0038 },
  { navn:'København',   lat:55.6761, lng:12.5683 },
  {                     lat:54.6996, lng:11.3543 },
  { navn:'Fehmarn',     lat:54.5033, lng:11.2242 },
  { navn:'Lübeck',      lat:53.8655, lng:10.6866 },
  ...
  { navn:'Karlsruhe',   lat:49.0069, lng:8.4037 },
  { navn:'Freiburg',    lat:47.9990, lng:7.8421 },
  { navn:'Basel',       lat:47.5596, lng:7.5886 },
  {                     lat:47.0502, lng:8.3093 },
  { navn:'Gotthard',    lat:46.5600, lng:8.5700 },
  {                     lat:46.1944, lng:9.0175 },
  {                     lat:45.8080, lng:9.0852 },
  { navn:'Milano',      lat:45.4642, lng:9.1900 },
  {                     lat:45.0526, lng:9.6929 },
  { navn:'Parma',       lat:44.8015, lng:10.3279 },
  { navn:'Bologna',     lat:44.4949, lng:11.3426 },
  { navn:'Firenze',     lat:43.7696, lng:11.2558 },
  { navn:'Siena',       lat:43.3188, lng:11.3308 },
  { navn:'Orvieto',     lat:42.7185, lng:12.1108 },
  { navn:'Roma',        lat:41.9028, lng:12.4964 }
];

/* =====================================================
   1b. AKTIVITETER
   Endre tallene her hvis dere vil vekte annerledes.
   Ny aktivitet legges til med én linje + én knapp i index.html.
   ===================================================== */
const FAKTOR = {
  lop:    { navn:'Løping',  vekt:1     },
  sykkel: { navn:'Sykling', vekt:1/3   }
};

/** Hvor mange kilometer denne økta flytter laget på ruta */
function effektiv(o){
  const vekt = FAKTOR[o.aktivitet]?.vekt ?? 1;   // eldre økter uten aktivitet = løping
  return Number(o.km || 0) * vekt;
}
...
async function lagLager(){
  if (firebaseConfig && firebaseConfig.projectId){
    try {
      ...
      const { getFirestore, collection, addDoc, setDoc, getDoc, deleteDoc, doc,
              onSnapshot, query, orderBy } =
        await import('https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js');

      const db  = getFirestore(initializeApp(firebaseConfig));
      const kol = collection(db, 'lag', lagId, 'okter');
      // Bildene ligger for seg selv, ikke i økt-dokumentet. Da slipper alle
      // å laste ned samtlige bilder hver gang de åpner siden.
      const bilde = id => doc(db, 'lag', lagId, 'bilder', id);
      ...
      return {
        ...
        async leggTil(o, bildedata){
          const ref = await addDoc(kol, o);
          if (bildedata) await setDoc(bilde(ref.id), { data: bildedata });
        },
        async slett(id){
          await deleteDoc(doc(kol, id));
          try { await deleteDoc(bilde(id)); } catch { /* fantes ikke */ }
        },
        async hentBilde(id){
          const d = await getDoc(bilde(id));
  ...
  const BILDER = 'bergen-roma-bilder-v1';
  let lyttere = [];
  ...
  const lesBilder  = () => JSON.parse(localStorage.getItem(BILDER) || '{}');
  const skrivBilder = o => localStorage.setItem(BILDER, JSON.stringify(o));
  ...
  return {
    ...
    async leggTil(o, bildedata){
      ...
      if (bildedata){
        const b = lesBilder(); b[id] = bildedata;
        try { skrivBilder(b); }
        catch { throw new Error('Nettleseren har ikke plass til flere bilder lokalt. Slå på delt logg, så lagres de i databasen.'); }
      }
      skriv([{ id, ...o }, ...les()]);
    },
    async slett(id){
      skriv(les().filter(o => o.id !== id));
      const b = lesBilder(); delete b[id]; skrivBilder(b);
    },
    async hentBilde(id){ return lesBilder()[id] || null; }
  };
}
...
/* =====================================================
   3. KART
   ===================================================== */
let kart = null, tilbakelagt = null, loper = null, byMarkorer = [];

/* Leaflet lastes fra unpkg i index.html. Svarer ikke den, prøver vi jsDelivr,
   slik at ett CDN som ligger nede ikke tar ned hele appen. */
async function sikreLeaflet(){
  ...
  await new Promise((ok, nei) => {
    ...
    s.onload = ok;
    s.onerror = () => nei(new Error('Fikk ikke lastet kartbiblioteket'));
    setTimeout(() => nei(new Error('Kartbiblioteket svarte ikke')), 8000);
    document.head.appendChild(s);
  });
  return !!window.L;
}

async function byggKart(){
  if (!await sikreLeaflet()) throw new Error('Kartbiblioteket mangler');

  kart = L.map('kart', { scrollWheelZoom:false, zoomControl:true });
  ...
  tilbakelagt = L.polyline([], { color:'#C08A2E', weight:4, opacity:.95 }).addTo(kart);
  kart.fitBounds(L.latLngBounds(linjePunkter).pad(0.06));

  byMarkorer = BYER.map(by =>
  ...
  loper = L.marker(posisjonEtter(0), {
...
try {
  ...
  // Kartet er fint å ha, men loggen skal virke uansett.
  const boks = document.getElementById('kart');
  if (boks) boks.innerHTML =
    '<p class="kart-feil">Kartet lastet ikke akkurat nå. Kilometerne registreres som normalt.</p>';
  console.error('Kart:', feil);
}
...
function tegn(okter){
  const total = okter.reduce((s,o) => s + effektiv(o), 0);
...
function tegnLag(okter){
  const per = new Map();
  okter.forEach(o => per.set(o.navn, (per.get(o.navn) || 0) + effektiv(o)));
...
function tegnLogg(okter){
  ...
  [...okter].sort((a,b) => (b.opprettet||0)-(a.opprettet||0)).slice(0,60).forEach(o => {
    ...
    const km = document.createElement('span');
    km.className = 'logg-km';
    const vekt = FAKTOR[o.aktivitet]?.vekt ?? 1;
    km.textContent = vekt === 1
      ? nf.format(o.km) + ' km'
      : `${nf.format(o.km)} km 🚲 → ${nf.format(effektiv(o))}`;
    km.title = vekt === 1 ? 'Løping' : `Sykling teller ${Math.round(1/vekt)} km per kilometer på ruta`;

    if (o.harBilde){
      const knapp = document.createElement('button');
      knapp.className = 'bilde-knapp';
      knapp.textContent = '📷';
      knapp.title = 'Se bildet';
      knapp.setAttribute('aria-label', `Se bildet ${o.navn} la ved`);
      knapp.onclick = () => visBilde(o);
      tekst.appendChild(knapp);
    }
...
/* --- Aktivitetsvalg --- */
let aktivitet = 'lop';
el('f-aktivitet')?.querySelectorAll('.valg-knapp').forEach(knapp => {
  knapp.onclick = () => {
    aktivitet = knapp.dataset.aktivitet;
    el('f-aktivitet').querySelectorAll('.valg-knapp')
      .forEach(k => k.classList.toggle('aktiv', k === knapp));
    const v = FAKTOR[aktivitet].vekt;
    const hjelp = el('valg-hjelp');
    if (hjelp) hjelp.textContent = v === 1
      ? '1 km løpt teller 1 km på ruta.'
      : `${Math.round(1/v)} km sykkel teller 1 km på ruta. Skriv inn hele turen — appen regner om.`;
  };
});

/* --- Bilde: krymp i nettleseren før lagring --- */
let valgtBilde = null;

async function krymp(fil, maksBredde = 900, kvalitet = 0.55){
  const bitmap = await createImageBitmap(fil);
  const skala  = Math.min(1, maksBredde / Math.max(bitmap.width, bitmap.height));
  ...
  bitmap.close?.();
  return lerret.toDataURL('image/jpeg', kvalitet);
}

if (el('f-bilde')) el('f-bilde').onchange = async e => {
  const fil = e.target.files[0];
  if (!fil) return;
  melding('Behandler bildet …', true);
  try {
    let data = await krymp(fil);
    if (data.length > 900000) data = await krymp(fil, 700, 0.4);
    if (data.length > 900000) throw new Error('Bildet er for stort selv etter komprimering.');
    valgtBilde = data;
    el('forhandsvis-bilde').src = data;
    el('forhandsvis').hidden = false;
    melding('');
  } catch (feil){
    valgtBilde = null;
    el('f-bilde').value = '';
    melding('Klarte ikke å lese bildet. Prøv et vanlig JPG- eller PNG-bilde.');
  }
};

function fjernBilde(){
  valgtBilde = null;
  if (el('f-bilde')) el('f-bilde').value = '';
  if (el('forhandsvis')) el('forhandsvis').hidden = true;
}
if (el('f-fjern-bilde')) el('f-fjern-bilde').onclick = fjernBilde;

/* --- Lysboks --- */
async function visBilde(o){
  if (!el('lysboks')) return;
  el('lysboks-tekst').textContent = 'Henter bildet …';
  el('lysboks-bilde').removeAttribute('src');
  el('lysboks').hidden = false;
  try {
    const data = await lager.hentBilde(o.id);
    if (!data) throw new Error('borte');
    el('lysboks-bilde').src = data;
    el('lysboks-bilde').alt = `Bilde lagt ved av ${o.navn}`;
    el('lysboks-tekst').textContent =
      [o.navn, o.dato, o.kommentar].filter(Boolean).join(' · ');
  } catch {
    el('lysboks-tekst').textContent = 'Fant ikke bildet.';
  }
}
function lukkLysboks(){
  if (!el('lysboks')) return;
  el('lysboks').hidden = true;
  el('lysboks-bilde').removeAttribute('src');
}
if (el('lysboks-lukk')) el('lysboks-lukk').onclick = lukkLysboks;
if (el('lysboks')) el('lysboks').onclick = e => { if (e.target === el('lysboks')) lukkLysboks(); };
window.addEventListener('keydown', e => { if (e.key === 'Escape') lukkLysboks(); });
...
el('f-send').onclick = async () => {
  ...
  if (!km || km <= 0) return melding('Skriv inn hvor mange kilometer du klarte.');
  if (km > 400)       return melding('Over 400 km på én økt? Del den opp i flere oppføringer.');
  if (!dato)          return melding('Velg en dato.');

  const for_ = visesNa;
  const paaRuta = km * FAKTOR[aktivitet].vekt;
  el('f-send').disabled = true;
  melding(valgtBilde ? 'Lagrer bildet …' : 'Lagrer …', true);
  try {
    await lager.leggTil({
      navn, km: Math.round(km*10)/10, dato, aktivitet,
    ...
    melding(FAKTOR[aktivitet].vekt === 1
      ? `${nf.format(km)} km lagt til. Bra jobba!`
      : `${nf.format(km)} km sykkel ble ${nf.format(paaRuta)} km på ruta.`, true);

    const passert = BYER.find(b => b.km > for_ && b.km <= for_ + paaRuta);
...
let sisteOkter = [];
el('f-csv').onclick = () => {
  const rader = [['Dato','Navn','Aktivitet','Kilometer','Km på ruta','Kommentar'],
    ...[...sisteOkter].sort((a,b) => (a.dato||'').localeCompare(b.dato||''))
      .map(o => [
        o.dato, o.navn,
        FAKTOR[o.aktivitet]?.navn || 'Løping',
        String(o.km).replace('.', ','),
        String(Math.round(effektiv(o)*10)/10).replace('.', ','),
        o.kommentar || ''
      ])];
...
/* =====================================================
   7. START
   ===================================================== */
window.addEventListener('error', e => visStorFeil(e.message));
window.addEventListener('unhandledrejection', e => visStorFeil(e.reason?.message || e.reason));

function visStorFeil(tekst){
  const s = el('status-tekst');
  if (s){
    s.textContent = 'Noe gikk galt: ' + tekst;
    s.style.color = 'var(--rust)';
  }
  console.error(tekst);
}
